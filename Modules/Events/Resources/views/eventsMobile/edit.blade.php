      ...
      $( window ).load(function() {
          $("#horizontal-pill-steps").steps({
            ...
            onStepChanged:function (event, currentIndex, newIndex) {
              if(currentIndex == 3){
                //setTimeout(function(){
                  var saveA = $("<a>").attr("href","#").addClass("saveBtn").text("save and accept");
                  var saveBtn = $("<li>").attr("aria-disabled",false).append(saveA);
                  var rejectA = $("<a>").attr("href","#").addClass("rejectBtn , bgcolor--fadebrown").text("cancel and reject");
                  var rejectBtn = $("<li>").attr("aria-disabled",false).append(rejectA);
                  var cancelA = $("<a>").attr("href","#").addClass("cancelBtn , bgcolor--fadegreen").text("cancel editing");
                  var cancelBtn = $("<li>").attr("aria-disabled",false).append(cancelA);
                  $(document).find(".actions ul").prepend(cancelBtn ,saveBtn ,rejectBtn )
                //},50);
              }else{
